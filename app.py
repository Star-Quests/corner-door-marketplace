...
# FORCE DATABASE CREATION ON RENDER
    ...
    if request.method == 'POST':
        ...
        if message.strip():
            ...
            if admin_user:
                ...
                create_notification(
                    admin_user.id,
                    f'≡ƒÆ¼ New message from {current_user.username}: {message[:50]}...',
                    None
                )
...
def admin_chat_reply():
    ...
    if user:
        ...
        create_notification(
            user_id,
            f'≡ƒÆ¼ Admin replied to your message: {message[:50]}...',
            None
        )
...
def checkout_cart():
    ...
    payment_message = f"""
≡ƒÄ» ORDER CONFIRMED - READY FOR PAYMENT

≡ƒÆ░ Total Amount: {crypto_amount:.8f} {crypto_type}
≡ƒôª Items in Order: {len(cart.items)}
≡ƒÅª Wallet Address: {wallet.address}

≡ƒôï Payment Instructions:
...
ΓÜá∩╕Å Important:
ΓÇó Send only {crypto_type} to this address
ΓÇó Amount must be exact
ΓÇó Contact support if issues occur
...
"""
    ...
    if admin_user:
        create_notification(
            admin_user.id,
            f'≡ƒ¢Æ New cart checkout: {len(cart.items)} items from {current_user.username} - Total: ${total_usd:.2f} USD',
...
≡ƒ¢ì∩╕Å Product: {product.title}
≡ƒÆ░ Amount: {crypto_amount:.8f} {crypto_type}
≡ƒÅª Wallet Address: {wallet.address}
...
"""
        ...
        if admin_user:
            create_notification(
                admin_user.id,
                f'≡ƒ¢ì∩╕Å New order #{order.id} for {product.title} by {current_user.username}',
                order.id
            )
...
def mark_paid(order_id):
    ...
    if admin_user:
        create_notification(
            admin_user.id,
            f'≡ƒÆ░ User marked order #{order.id} as paid. Please verify {order.crypto_amount:.8f} {order.crypto_type} payment.',
            order.id
        )
    
    create_notification(
        current_user.id,
        f'Γ£à Payment marked as sent! Admin will verify your {order.crypto_amount:.8f} {order.crypto_type} payment for order #{order.id}.',
        order.id
    )
...
def admin_delete_product(product_id):
    ...
    try:
        ...
        db.session.rollback()
        print(f"Γ¥î Delete product error: {str(e)}")
...
def admin_deliver_order(order_id):
    ...
    if request.method == 'POST':
        ...
        create_notification(
            order.user_id,
            f'≡ƒÄë ORDER #{order.id} DELIVERED!\n\nYour product "{order.product.title}" is ready for download.\n\n≡ƒôÑ Click "Download Product" to get your files.\nΓ¡É Don\'t forget to rate your purchase!',
            order.id
        )
...
def create_first_admin():
    if not User.query.filter_by(username='corner').first():
        ...
        db.session.commit()
        print("≡ƒÄë PRIMARY ADMIN CREATED: username='corner', password='cornerdooradmin4life'")

if __name__ == '__main__':
    with app.app_context():
        print("≡ƒöä INITIALIZING DATABASE...")
        db.create_all()
        create_first_admin()
        ...
        if admin:
            print(f"Γ£à ADMIN VERIFIED: {admin.username} (Active: {admin.is_active}, Admin: {admin.is_admin})")
    
    print("")
    print("≡ƒÜÇ CORNER DOOR MARKETPLACE STARTED!")
    print("≡ƒöæ ADMIN LOGIN:")
    print("   Username: corner")
    print("   Password: cornerdooradmin4life")
    print("≡ƒîÉ ACCESS AT: http://localhost:5000")
    print("")
